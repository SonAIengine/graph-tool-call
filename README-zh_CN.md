...
- **248 个工具**: LLM 崩溃到 20%。graph-tool-call 以 72-76% 准确率救场 — 这不是优化，而是**必需品**。
...
| Pipeline | Accuracy | Recall@K | 平均 Token | Token 节省 |
|----------|:--------:|:--------:|:---------:|:---------:|
| baseline (全部 248 tools) | 20.0% | 100.0% | 8,192 | — |
| retrieve-k5 | 72.0% | 86.0% | 1,614 | 80% |
| + 嵌入 | **76.0%** | **94.0%** | 1,720 | 79% |
| + 本体 | 74.0% | 90.0% | 1,707 | 79% |
| + 两者 | 74.0% | **96.0%** | 1,901 | 77% |
...
|----------|:--------:|:--------:|------|
| retrieve-k5 | 72.0% | 86.0% | 仅 BM25 + 图 |
| + 嵌入 | **76.0%** | **94.0%** | + OpenAI 嵌入 |
| + 本体 | 74.0% | 90.0% | + GPT-4o-mini 知识图谱 |
| **+ 两者** | 74.0% | **96.0%** | **嵌入 + 本体** |

- **嵌入**: Recall@5 **86% → 94%** (+8pp), 准确率 **72% → 76%** — 捕获 BM25 遗漏的语义匹配。
- **本体**: Recall@5 **86% → 90%** (+4pp), 准确率 **72% → 74%** — LLM 丰富的关键词改善 BM25 评分。
- **两者结合**: Recall@5 **96%**（最高）— 本体关键词 + 嵌入语义互补。
