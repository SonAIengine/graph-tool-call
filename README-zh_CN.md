...
| API | 工具数 | 方式 | 准确率 | Recall@5 |
|-----|:-----:|------|:------:|:--------:|
| Petstore | 19 | 不使用 graph-tool-call（全部 19 个） | 100% | — |
| GitHub | 50 | 不使用 graph-tool-call（全部 50 个） | 100% | — |
| MCP Servers | 38 | 不使用 graph-tool-call（全部 38 个） | 96.7% | — |
| **Kubernetes** | **248** | **不使用 graph-tool-call（全部 248 个）** | **12%** | — |
| | | **使用 graph-tool-call（筛选前 5 个）** | **78%** | **91%** |
| | | + 嵌入 | **80%** | **94%** |
| | | + 本体 | **82%** | **96%** |
| | | + 两者 | **82%** | **98%** |
