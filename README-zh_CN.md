...
在 BM25 + 图遍历基础上添加嵌入模型（[nomic-embed-text](https://ollama.com/library/nomic-embed-text), Ollama）的结果 — 效果取决于工具数量：

| API | 工具数 | BM25 + Graph | + 嵌入 | 改善 | 退化 |
|-----|:------:|:------------:|:------:|:----:|:----:|
...
**规律**: 中小规模（< 100 个工具）中，BM25 关键词匹配已经足够精确 — 添加嵌入反而会引入噪声，可能将正确工具推出 top-5。但在**大规模（248+）**中，许多工具名称相似（`readCoreV1NamespacedPodStatus` vs `connectCoreV1GetNamespacedPodAttach`），BM25 无法区分，嵌入的语义理解才能发挥真正价值。

**建议**: 工具数量超过约 100 个时启用嵌入。少于此数时，BM25 + 图遍历已经足够。
