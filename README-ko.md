...
- **248개**: LLM이 **20%로 붕괴**. graph-tool-call이 **72–76% 정확도** 달성 — 최적화가 아니라 **필수**.
...
| + 임베딩 | **76.0%** | **94.0%** | 1,720 | 79.0% |
| + 온톨로지 | 74.0% | 90.0% | 1,707 | 79.2% |
| + 둘 다 | 74.0% | **96.0%** | 1,901 | 76.8% |
...
|----------|:------:|:--------:|------|
| retrieve-k5 | 72.0% | 86.0% | BM25 + 그래프만 |
| + 임베딩 | **76.0%** | **94.0%** | + OpenAI 임베딩 |
| + 온톨로지 | 74.0% | 90.0% | + GPT-4o-mini 지식그래프 |
| **+ 둘 다** | 74.0% | **96.0%** | **임베딩 + 온톨로지** |

- **임베딩**: Recall@5 **86% → 94%** (+8pp), 정확도 **72% → 76%** — BM25가 놓치는 의미적 유사성을 포착.
- **온톨로지**: Recall@5 **86% → 90%** (+4pp), 정확도 **72% → 74%** — LLM 보강 키워드가 BM25 스코어링 개선.
- **둘 다 결합**: Recall@5 **96%** (최고) — 온톨로지 키워드 + 임베딩 의미론이 상호 보완.
