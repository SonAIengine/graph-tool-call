...
> 정확도 = LLM이 50개 테스트 쿼리에서 정답 tool을 고른 비율

| API | Tool 수 | graph-tool-call ✗ | graph-tool-call ✓ | 토큰 절감 |
|-----|:------:|:-----------------:|:-----------------:|:---------:|
...
- **graph-tool-call ✗** = 전체 tool을 LLM에 그대로 전달
- **graph-tool-call ✓** = 검색으로 관련 tool만 추려서 전달
- **50개 미만**: LLM이 전체를 줘도 잘 고름. graph-tool-call 가치 = **64–88% 토큰 절감**.
- **248개**: 전체를 주면 **12%로 붕괴**. 필터링하면 **82%** — 최적화가 아니라 **필수**.
...
| Pipeline | 정확도 | Recall@5 |
|----------|:------:|:--------:|
| graph-tool-call 없이 (248개 전부) | 12% | — |
| graph-tool-call 사용 (상위 5개) | 78% | 91% |
| + 임베딩 | 80% | 94% |
| + 온톨로지 | 82% | 96% |
| + 둘 다 | **82%** | **98%** |
