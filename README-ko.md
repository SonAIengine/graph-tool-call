...
여러 MCP 서버를 하나의 proxy로 묶습니다. **Gateway 모드** (tool 30개 초과 시)에서는 3개 meta-tool만 노출돼서 턴당 context ~1,500 → ~300 토큰으로 절감됩니다:
...
`search_tools` 호출 후 매칭된 tool이 **동적으로 tool list에 등록**돼서 1-hop 직접 호출이 가능합니다.
