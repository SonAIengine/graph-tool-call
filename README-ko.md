...
### Swagger에서 자동 생성 (Phase 1)

```python
tg = ToolGraph()
tg.ingest_openapi("https://petstore.swagger.io/v2/swagger.json")

# 자동 처리: 20개 endpoint → 20개 tool → 34개 관계 → 3개 카테고리
# CRUD 의존성, 호출 순서, 카테고리 그룹핑 — 모두 자동 감지.
...
| Phase | 내용 | 상태 |
|-------|------|------|
| **0** | 핵심 그래프 엔진 + 하이브리드 검색 | ✅ 완료 (32 tests) |
| **1** | OpenAPI 수집, spec 정규화, 의존성 & 순서 감지 | 진행 중 |
