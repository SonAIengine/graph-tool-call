...
BM25 + 그래프 위에 임베딩 모델([nomic-embed-text](https://ollama.com/library/nomic-embed-text), Ollama)을 추가한 결과 — tool 수에 따라 효과가 달랐습니다:

| API | Tool 수 | BM25 + Graph | + 임베딩 | 개선 | 저하 |
|-----|:------:|:------------:|:-------:|:----:|:----:|
...
**패턴**: 소/중 규모(100개 미만)에서는 BM25 키워드 매칭이 이미 충분히 정확합니다 — 임베딩을 추가하면 오히려 노이즈가 생겨 정답 tool이 top-5 밖으로 밀릴 수 있습니다. 반면 **대규모(248개 이상)**에서는 비슷한 이름의 tool이 많아(`readCoreV1NamespacedPodStatus` vs `connectCoreV1GetNamespacedPodAttach`) BM25만으로 구별이 안 되고, 임베딩의 시맨틱 이해가 진가를 발휘합니다.

**권장**: tool 수가 ~100개를 넘으면 임베딩 활성화. 그 이하에서는 BM25 + 그래프만으로 충분합니다.
