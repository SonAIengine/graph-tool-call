...
# MCP tool list 수집 (annotation 보존)
mcp_tools = [
    {
        "name": "read_file",
        "description": "파일 읽기",
...
# "파일 삭제" → destructive 도구가 상위 랭크 (annotation-aware)
tools = tg.retrieve("임시 파일 삭제", top_k=5)
```

MCP annotation (`readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`)이 retrieval signal로 활용됩니다. query intent가 자동 분류되어 tool annotation과 alignment됩니다 — 조회 쿼리는 read-only 도구를, 삭제 쿼리는 destructive 도구를 우선합니다.
...
| 검색 방식 | 단순 벡터 유사도 | 그래프 + 벡터 하이브리드 (wRRF), 3-Tier |
...
| **2** | 중복 제거, 임베딩, 온톨로지 모드 (Auto/LLM-Auto), 검색 Tier, `from_url()` | ✅ 완료 (181 tests) |
| **2.5** | MCP Annotation-Aware Retrieval: intent classifier, annotation scoring, wRRF 통합 | ✅ 완료 (255 tests) |
| **3** | Pyvis 시각화, Neo4j export, CLI, PyPI 배포 | 계획됨 |
