...
|----------|:------:|:--------:|------|
| retrieve-k5 | 70.0% | 86.0% | BM25 + 그래프만 |
| + 임베딩 | 70.0% | **94.0%** | + OpenAI 임베딩 |
| + 온톨로지 | 68.0% | 86.0% | + GPT-4o-mini 지식그래프 |
| **+ 둘 다** | **72.0%** | **94.0%** | **임베딩 + 온톨로지** |

- **임베딩**: Recall@5 **86% → 94%** (+8pp) — BM25가 놓치는 의미적 유사성을 포착.
- **온톨로지 단독**: 구조적 API(K8s tool 이름이 이미 서술적)에서는 효과 미미.
- **둘 다 결합**: 최고 정확도(72%) — 임베딩만으로 놓치는 엣지 케이스를 온톨로지가 보완 (예: "List all endpoints for a service" → `full` 파이프라인만 정답).
