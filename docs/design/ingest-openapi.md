...
fallback 경로로만 사용한다.
PathSynthesizer는 `response_collection_path`/`response_envelope_path`가 있는
producer row를 chain binding으로 사용할 때 HttpExecutor의
`body_view.value` 경로를 우선 선택한다. 예를 들어
`$.data.items[*].goodsNo`는 `${s1.body_view.value[0].goodsNo}`로 바인딩된다.
