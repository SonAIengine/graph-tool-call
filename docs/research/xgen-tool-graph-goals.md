...
기준선은 qwen3.6-27B, BFCL v4 four-category local BFCL-compatible evaluator
기준이다. 현재 표의 BFCL 수치는 0.27 제품 후보를 빠르게 판정하기 위한 T3 artifact
`/tmp/gtc-bfcl-date-guidance-limit25-repeat3-row-retrieved-qwen.json` 기준이다.
범위는 `simple_python,multiple,parallel,parallel_multiple` 각 25건, row/retrieved
source, top-k=5, repeat 3이다. XGEN 적용성 기준선은 X2BEE BO Swagger UI live
acceptance run을 별도로 둔다.

| Metric | Current |
|---|---:|
| BFCL artifact scope | `4 categories x 25 cases x 3 repeats` |
| BFCL row-source exact@5 mean/std | `1.00 / 0.000` |
| graph-tool-call retrieved exact@5 mean/std | `1.00 / 0.000` |
| graph-tool-call retrieved retrieval@5 | `1.00` |
| row-source upper-bound preservation | `1.00` |
| parallel_multiple exact@5 | `1.00` |
| row-pass retrieved-fail cases | `0 / 300 paired cases` |
| xgen-0.27 milestone gate | `pass` |
| xgen-0.28 full BFCL gate | `fail` |
| xgen-0.28 full retrieved exact@5 | `0.865` |
| xgen-0.28 full retrieved retrieval@5 | `0.953` |
| xgen-0.28 full row preservation | `0.904497` |
| xgen-0.28 full parallel_multiple exact@5 | `0.730` |
| last broad 1000-case retrieved exact@5 | `0.764` |
| last broad 1000-case retrieved exact@10 | `0.798` |
...
현재 상태는 "감이 아니라 수치로 검증 가능한 XGEN 적용 기준선"이다. 0.27 제품 후보
gate는 row-source와 retrieved-source repeat-3 모두에서 통과했고, row-pass인데
retrieved만 실패한 paired loss가 0건이다. 다만 이 수치는 full public benchmark가
아니라 100-case T3 milestone evidence다. Paper-ready로 가려면 최신 구현으로 broad
1000-case full distribution, confidence interval, official checker 재검증을 다시
돌려야 한다. XGEN deterministic fixture에서는
...
`2026-07-19` qwen3.6-27B full BFCL paper-ready artifact
`/tmp/gtc-bfcl-028-gate-full-qwen.json`은 `xgen-0.28` gate를 통과하지 못했다.
Scope는 `simple_python,multiple,parallel,parallel_multiple` 전체 1000 case,
row/retrieved, top-k `5`, repeat `3`이다. Mean retrieved exact@5는 `0.865`
로 threshold `0.87`에 근접했지만, retrieval@5 `0.953 < 0.96`,
row-source upper-bound preservation `0.904497 < 0.96`,
`parallel_multiple` exact@5 `0.730 < 0.78` 때문에 `fail`이다. Failure inspector
`/tmp/gtc-bfcl-028-failure-inspect.json`은 91개 retrieved failure case에서
expected tool mention 169개를 분석했고, `all_expected_found_at_k=0.274725`,
`all_expected_found_at_depth=0.901099`로 top-5 rank compression이 1차 병목임을
보였다. Issue breakdown은 `reported_retrieval_miss=69`,
`expected_present_below_top_k=61`, `partial_multi_tool_at_k=36`,
`reported_candidate_ambiguity=22`, `some_expected_outside_inspect_depth=5`,
`all_expected_outside_inspect_depth=4`, `weak_or_missing_keyword_signal=2`다.
따라서 0.28 연구의 다음 타깃은 top-k를 단순히 늘리는 것이 아니라, multi-tool set
보존, sibling ambiguity 억제, optional embedding/rerank gate를 통해 top-5 안에
정답 tool set 전체를 보존하는 것이다.
...
`2026-07-20` BFCL domain-alias branch에서는 history/religion/grocery/music/
instrument/sports/physics/restaurant/recipe/stock/hospital 도메인의 sparse query를
guarded alias expansion과 semantic phrase boost로 보강했다. 각 alias는 도메인
명사와 task intent가 동시에 있을 때만 켜지며, `NBA ranking` query가
`match_schedule` boost를 잘못 받지 않도록 schedule intent guard와 negative
regression test를 추가했다. T1 deterministic artifact
`/tmp/gtc-research-check/bfcl-deterministic.json` 기준 `recall@5`는
`0.95200 -> 0.956917`, `all_tools_found@5`는 `0.930 -> 0.933`,
`argument_schema_coverage`는 `0.929256`, 평균 latency는 `24.08ms`이고 status는
`pass`다. 같은 T1 run의 XGEN deterministic artifact
`/tmp/gtc-research-check/xgen-deterministic.json`도 status `pass`,
target selector exact `1.00`, plan/binding/evidence coverage `1.00`,
평균 candidate count `2.60`을 유지했다. 이 결과는 deterministic retrieval gate
...
|---|---|---|
| Current | 0.27 gate-pass 제품 후보 | XGEN 기본 경로 후보 검토, paper-ready 확장 검증 |
...
- row-source upper bound 대비 top-k=5 exact `0.85` 이상이면 검색 레이어가 모델
  성능을 크게 깎지 않는 제품 후보로 본다.
...
|---|---:|---|
| BFCL-compatible retrieved exact@5 | `1.00` | pass |
| retrieval@5 | `1.00` | pass |
| row-source upper-bound preservation | `1.00` | pass |
| parallel_multiple exact@5 | `1.00` | pass |
| XGEN fixture coverage | `commerce/admin/workflow` | pass |
| X2BEE target selector exact@5 | `1.00` | pass |
| X2BEE p50 retrieval latency | `29.86ms` | pass |

이 status는 `/tmp/gtc-bfcl-date-guidance-limit25-repeat3-row-retrieved-qwen.json`과
`/tmp/gtc-x2bee-schema-context-threshold-sweep.json` 기준이다. 다음 승격은 이
제품 후보 증거를 full 1000-case BFCL rerun, multi-dataset OpenAPI fixture,
ablation/CI 포함 paper-ready evidence로 넓히는 것이다.
...
verifies the manifest paths and sha256 values before expensive acceptance runs. Snapshot
manifests store spec paths relative to the snapshot directory, so the captured folder can
move as a unit. Reports produced with `MANIFEST=...` keep `snapshot_manifests`
provenance so research claims can point back to exact spec hashes.
