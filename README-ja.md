...
BM25 + グラフの上にエンベディングモデル（[nomic-embed-text](https://ollama.com/library/nomic-embed-text), Ollama）を追加した結果 — 効果はツール数によって異なりました：

| API | ツール数 | BM25 + Graph | + エンベディング | 改善 | 低下 |
|-----|:------:|:------------:|:--------------:|:----:|:----:|
...
**パターン**: 小・中規模（100個未満）では、BM25キーワードマッチングがすでに十分正確です — エンベディングを追加するとノイズが生じ、正解ツールがtop-5から外れる可能性があります。一方、**大規模（248個以上）**では、類似した名前のツールが多く（`readCoreV1NamespacedPodStatus` vs `connectCoreV1GetNamespacedPodAttach`）、BM25だけでは区別できず、エンベディングのセマンティックな理解が真価を発揮します。

**推奨**: ツール数が約100個を超える場合にエンベディングを有効化。それ以下ではBM25 + グラフで十分です。
