...
**パターン**: 小・中規模では、BM25キーワードマッチングがすでに十分正確です — ツール名がクエリキーワードと直接マッチする場合（例: "look up user" → `getUser`）、エンベディングがかえって邪魔になります。一方、**大規模（248個以上）**では、類似した名前のツールが多く（`readCoreV1NamespacedPodStatus` vs `connectCoreV1GetNamespacedPodAttach`）、BM25だけでは区別できず、エンベディングのセマンティックな理解が真価を発揮します。

<details>
<summary>モデル品質が重要です</summary>

同じテストをnomic-embed-textで行うと、結果はより悪くなります — 低下が多く、改善が少ない：

| API | ツール数 | nomic-embed-text | Qwen3-Embedding-0.6B |
|-----|:------:|:----------------:|:--------------------:|
| MCP | 38 | 90.0% (↓2) | **96.7%** (↓0) |
| GitHub | 58 | 77.5% (↓3) | **80.0%** (↓2) |
| K8s | 248 | 66.0% (↑1) | **68.0%** (↑2) |

良いエンベディングモデル = 小規模でのノイズ減少 + 大規模でのより大きな改善。

</details>

**推奨**: ツール数が約100個を超える場合にエンベディングを有効化。それ以下ではBM25 + グラフで十分です。エンベディングを有効化する場合は、高品質なモデルを使用してください — 測定可能な差があります。
