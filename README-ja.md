...
| 指標 | 何を測定するか | 例 |
|------|-------------|------|
| **精度 (Accuracy)** | LLMが正しいツールを選んだか？ | "pod一覧" → LLMが `listCoreV1NamespacedPod` を選択 → 正解 |
| **Recall@K** | 正解ツールが候補リストに含まれていたか？ | `listCoreV1NamespacedPod` が上位5件に含まれる → はい |

> **両方重要な理由**: 正解ツールが候補にない場合（低Recall）、LLMがどんなに賢くても選べません。候補にあるのにLLMが間違えた場合（低Accuracy）、それは検索ではなくLLMの選択の問題です。

### 核心的発見：ツールが多すぎるとLLMが混乱する

| API | ツール数 | 方式 | 精度 | Recall@5 |
|-----|:-----:|------|:----:|:--------:|
| Petstore | 19 | graph-tool-callなし（全19件） | 100% | — |
| GitHub | 50 | graph-tool-callなし（全50件） | 100% | — |
| MCP Servers | 38 | graph-tool-callなし（全38件） | 96.7% | — |
| **Kubernetes** | **248** | **graph-tool-callなし（全248件）** | **12%** | — |
| | | **graph-tool-call使用（上位5件選別）** | **78%** | **91%** |
| | | + エンベディング | **80%** | **94%** |
| | | + オントロジー | **82%** | **96%** |
| | | + 両方 | **82%** | **98%** |

**Kubernetesで何が起きたか？**
- **Baseline（全248ツールを渡す）**: LLMが248のツール定義を一度に見ます。情報過多で混乱し、88%の確率で間違える → **精度12%**。（Recallは技術的に100% — 正解はリストに*ある*が、LLMが見つけられない。）
- **graph-tool-callを使用**: 関連性の高い5件にフィルタリング。LLMの正答率が**78–82%**に。最適化ではなく——**使えるか使えないかの違い**。

**50件以下**: LLMは問題なく処理できる。graph-tool-callは**64–88%のトークン削減**効果（より速く、より安く）。
